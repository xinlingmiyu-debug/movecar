...
const CONFIG = { 
  KV_TTL: 3600,
  R2_PUBLIC_URL: 'https://images.xyzvip.cn' // ✅ 改为你的新域名
}

// 获取高德API密钥
function getAmapApiKey() {
  try {
    return typeof AMAP_API_KEY !== 'undefined' ? AMAP_API_KEY : '';
  } catch (e) {
    return '';
  }
}
...
async function handleUploadImage(request) {
  try {
    // 检查R2是否绑定
    if (typeof MOVE_CAR_IMAGES === 'undefined') {
      return new Response(JSON.stringify({ 
        success: false, 
        error: 'R2存储未配置，请检查Cloudflare R2绑定',
    ...
    if (!allowedTypes.includes(file.type)) {
      return new Response(JSON.stringify({ error: '不支持的文件格式，请上传 JPG/PNG/GIF/WEBP' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' }
      });
    }
    ...
    if (file.size > 5 * 1024 * 1024) {
      return new Response(JSON.stringify({ error: '图片大小不能超过5MB' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' }
      });
    }
    ...
    const key = `movecar/${fileName}`;

    const buffer = await file.arrayBuffer();

    await MOVE_CAR_IMAGES.put(key, buffer, {
      httpMetadata: {
    ...
    const imageUrl = `${CONFIG.R2_PUBLIC_URL}/${key}`;
...
async function reverseGeocode(lat, lng) {
  ...
  try {
    ...
    const url = `https://restapi.amap.com/v3/geocode/regeo?location=${gcj.lng},${gcj.lat}&key=${apiKey}&output=json&radius=100&extensions=base`;
    ...
    if (data.status === '1' && data.regeocode) {
      ...
      if (!address) {
        ...
        address = parts.join('') || `位置 (${lat.toFixed(6)}, ${lng.toFixed(6)})`;
      }
...
async function handleNotify(request, url) {
  try {
    ...
    if (location && location.lat && location.lng) {
      locationDescription = await reverseGeocode(location.lat, location.lng);
      notifyBody += `\n📍 位置: ${locationDescription}`;
      ...
      Object.assign(locationData, {
        ...
        amapUrl: urls.amapUrl,
        appleUrl: urls.appleUrl,
        baiduUrl: urls.baiduUrl,
        tencentUrl: urls.tencentUrl
      });
    } else {
      notifyBody += '\n⚠️ 未提供位置信息';
    }
    ...
    // 如果有图片，使用正确的域名添加到通知中
    if (imageUrl) {
      const finalImageUrl = imageUrl.startsWith('http') ? imageUrl : `${CONFIG.R2_PUBLIC_URL}/${imageUrl}`;
      notifyBody += `\n📷 现场照片: ${finalImageUrl}`;
      locationData.imageUrl = finalImageUrl;
      await MOVE_CAR_STATUS.put('request_image', finalImageUrl, { expirationTtl: CONFIG.KV_TTL });
    }

    if (Object.keys(locationData).length > 0) {
      await MOVE_CAR_STATUS.put('requester_location', JSON.stringify(locationData), { expirationTtl: CONFIG.KV_TTL });
    }
    ...
    if (typeof BARK_URL !== 'undefined' && BARK_URL && BARK_URL.trim() !== '') {
      pushTasks.push(
        (async () => {
          try {
            await sendBark(notifyBody, confirmUrl);
          } catch (e) {
    ...
    if (typeof TG_BOT_TOKEN !== 'undefined' && TG_BOT_TOKEN && TG_BOT_TOKEN.trim() !== '' && 
        typeof TG_CHAT_ID !== 'undefined' && TG_CHAT_ID && TG_CHAT_ID.trim() !== '') {
      pushTasks.push(
        (async () => {
          try {
    ...
    if (typeof PUSHPLUS_TOKEN !== 'undefined' && PUSHPLUS_TOKEN && PUSHPLUS_TOKEN.trim() !== '') {
      pushTasks.push(
        (async () => {
          try {
...
async function handleOwnerConfirmAction(request) {
  try {
    ...
    if (ownerLocation) {
      const address = await reverseGeocode(ownerLocation.lat, ownerLocation.lng);
  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; margin: 0; padding: 0; }
      ...
      .preview-area .remove-btn:active { transform: scale(0.9); }
    ...
    <div class="container" id="mainView">
      ...
      <div class="card upload-card" id="uploadCard">
        <div class="upload-area" id="dropArea" onclick="document.getElementById('fileInput').click()">
          ...
          <div class="text">点击或拖拽上传现场照片</div>
          <div class="sub-text">支持 JPG, PNG, GIF, WEBP (最大5MB)</div>
        </div>
        ...
        <div id="uploadStatus" style="font-size:12px;color:#718096;text-align:center;display:none;"></div>
      </div>
      ...
      function startCooldown(seconds) {
        ...
        cooldownTimer = setInterval(() => {
          ...
          if (cooldownSeconds <= 0) {
            ...
            display.textContent = \`⏳ 冷却中：\${cooldownSeconds}秒后可再次通知\`;
          }
        }, 1000);
      }
...
function renderOwnerPage() {
  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
      ...
      .image-section.show { display: block; }
      .image-section img {
        width: 100%;
        max-height: 250px;
        object-fit: cover;
  ...
  <body>
    <div class="card">
      ...
      <h1>收到挪车请求</h1>
      <p class="subtitle" id="requestInfo">对方正在等待，请尽快确认</p>

      <div id="imageSection" class="image-section">
        <div class="label">📷 现场照片</div>
        <img id="sceneImage" src="" alt="现场照片">
      </div>
        ...
        try {
          ...
          if(res.ok) {
            ...
            if(data.imageUrl) {
              ...
              document.getElementById('imageSection').classList.add('show');
              document.getElementById('sceneImage').src = data.imageUrl;
            }
          }
        } catch(e) {
...
async function sendBark(notifyBody, confirmUrl) {
  const barkApiUrl = `${BARK_URL}/挪车请求/${encodeURIComponent(notifyBody)}?group=MoveCar&level=critical&call=1&sound=minuet&icon=https://cdn-icons-png.flaticon.com/512/741/741407.png${confirmUrl ? '&url=' + confirmUrl : ''}`;
